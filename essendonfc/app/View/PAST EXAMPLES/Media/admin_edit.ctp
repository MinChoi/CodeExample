...
?>
<?= $this->Form->create(null,array('url'=>'/admin/media/edit/'.$this->request->data['Media']['id'],'id'=>'mediaadd','type'=>'file'));?>
<?=$this->Form->input('Media.id');?>
<?=$this->Form->input('Media.meta_id',array('type'=>'hidden'));?>
<?=$this->Form->input('Redirect.redirect',array('type'=>'hidden'));?>
<div class="frm-l">
	<h3><address>Edit Media</address></h3>
	<ol class="erroruls">
		<? 
		if(isset($errors)){
		...
		<tr>
			...
			<td>
				...
				<div id="mediaimg" style="display:block;">
					...
					?>
					<img src="<?=$src;?>" alt="Preview Image" width="<?=($src!='/img/blank.png')?'':'1px'?>" id="media_preview_file" />&nbsp;&nbsp;<a class="editlinks" href="#" onclick="media_img_edit();return false;">Edit</a>
				</div>
