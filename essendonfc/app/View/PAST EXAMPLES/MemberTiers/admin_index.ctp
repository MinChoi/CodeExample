...
<div id="index-block" style="clear:both;">
	<div id="indexrows">
		<?
		$i = 0;
		foreach ($memberTiers as $memberTier):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
		?>
			<div id="rowid_<?= $memberTier['MemberTier']['id']; ?>" class="row">
				<div class="rowitem" style="width:37%">
					<div class="title">
					<?= $memberTier['MemberTier']['name']; ?>
					</div>
				</div>
				<div class="rowitem" style="width:13%;padding-left:20px;"><div class="rowcell">
					<b><?= $number->currency($memberTier['MemberTier']['price'], 'USD'); ?></b>
					<span> inc GST</span>
				</div></div>
				<div class="rowitem" style="width:10%;padding-left:20px;"><div class="rowcell">
					<span class="green"><?= $memberTier['MemberTier']['members_count']; ?></span><br />
					<span>members</span>
				</div></div>
				<div class="rowitem" style="width:16%;padding-left:20px;"><div class="rowcell">
					<span class="yellow"><?= $memberTier['MemberTier']['grace_period_count']; ?></span><br />
					<span>in grace period</span>
					<? /*if(intval($memberTier['MemberTier']['approval_required'])==1){?>
						<span class="yellow"><?= $memberTier['MemberTier']['pending_count']; ?></span><br />
						<span>pending</span>
					<? }else{?>
				...
				<div class="rowitem" style="width:10%;padding-left:20px;"><div class="rowcell">
					<? 
						echo (intval($memberTier['MemberTier']['published'])==1)?'<div id="pub_'.$memberTier['MemberTier']['id'].'" class="published">Published</div>':'<div id="pub_'.$memberTier['MemberTier']['id'].'" class="unpublished">Unpublished</div>';
					?>
				</div></div>
			</div>
		<? endforeach; ?>
	...
	<form id="memgroupfrm" onsubmit="return false;">	
		<div class="slb_rbox">
			...
			<div class="slb_rbox-content">
				<?
					echo $this->Form->input('MemberTier.id',array('type'=>'hidden'));
					echo $this->Form->input('MemberTier.name',array('style'=>'width:335px;','label'=>'Tier Name'));
					echo $this->Form->input('MemberTier.price',array('style'=>'width:100px;text-align:right;','label'=>'Tier Cost','between'=>'<b style="color:black;
...
?>
	...
	<script type="text/javascript">
		...
		function updateMemGroup(){
			ele = $('rowid_'+$('MemberTierId').value.trim());
			ele.down(1).update($('MemberTierName').value);
			ele.down(0).next(0).down(1).update('$'+addCommas($('MemberTierPrice').value));
			hideStyleBox('memgedit');
			new Ajax.Request('/admin/member_tiers/ajaxupdate/',
			  {
				method:'post',
				parameters: { 
					'id' : $('MemberTierId').value,
					'name': $('MemberTierName').value,
					'price': $('MemberTierPrice').value
				},
		...
		function processRows()
		{
			...
			record_rows.each(function(e){
				...
				e.observe('dblclick', function(event){			
					...
					$('MemberTierId').value = $id;
					$('MemberTierName').value = ele.down(1).innerHTML.trim();
					$('MemberTierPrice').value =  ele.down(0).next(0).down(1).innerHTML.trim().gsub(',', '').gsub('$','');
		...
		function doGAction(action)
		{
			...
			$selectedids.each(function(id){
				new Ajax.Request('/admin/member_tiers/'+action+'/'+id.gsub('rowid_', ''),
				  {
					method:'get',
					onSuccess: function(transport){
		...
		{
			new Ajax.Request('/admin/member_tiers/'+action+'/'+id,
			  {
				method:'get',
				onSuccess: function(transport){
				  var response = transport.responseText || "no response text";
