...
function attachToSelect(selectEle,optionArr){
	$(selectEle).update('');
	optionArr.each(function(key,val){   
		$(selectEle).insert(new Element('option', {value: val}).update(key));
	});
}
...
document.observe("dom:loaded", function(){
	...
	var $country_iArr = ['OrganisationStreetCountryId','OrganisationPostalCountryId','OrganisationKcCountryId'];
	$country_iArr.each(function(e){   

		$(e).observe('change', function(event){
			var unique_id =  Event.element(event).id;
			var $states = aStates[$(unique_id).value].split('|');
			switch(unique_id){
